import { ConceptLink, Lecture } from "@/lib/types";
const date = (n: number) => new Date(n).toISOString().slice(0, 10);
export const describeLink = (link: ConceptLink) => `${link.from} ${link.relation === "contrast" ? "↔" : link.relation === "definition" ? "=" : link.relation === "category" ? "⊃" : link.relation === "unclear" ? "…" : "→"} ${link.relation === "example" ? "예: " : ""}${link.to} (${link.relation})`;
export function lectureToMarkdown(l: Lecture) {
  const note = l.finalNote;
  const links = l.conceptLinks ?? l.stateSnapshots.at(-1)?.state.conceptLinks ?? [];
  const flow = links.length ? links.map(describeLink).join("\n") : (note?.overview ?? l.stateSnapshots.at(-1)?.state.conceptChain ?? []).join("\n→ ");
