...
import { completedOutputText, parseStructuredResponse, ResponseProcessingError } from "@/lib/response-processing";
const NOTE_SCHEMA = {
  type: "object",
...
const summarizeChunk = async (text: string) => {
  ...
  const run = async (tokens: number) => completedOutputText(await openaiFetch("/responses", { ...body, max_output_tokens: tokens }, 30_000));
  try { return await run(700); }
  catch (error) {
    if (error instanceof ResponseProcessingError && error.stage === "openai_incomplete" && error.reason === "max_output_tokens") return run(1100);
    throw error;
  }
};
export async function POST(request: Request) {
  ...
  try {
    ...
    const body = {
      ...
      text: { format: { type: "json_schema", name: "lecture_note", strict: true, schema: NOTE_SCHEMA } },
    };
    const run = async (tokens: number) => parseStructuredResponse(await openaiFetch("/responses", { ...body, max_output_tokens: tokens }, 50_000), finalNoteSchema);
    try { return Response.json(await run(3000)); }
    catch (error) {
      if (error instanceof ResponseProcessingError && error.stage === "openai_incomplete" && error.reason === "max_output_tokens") return Response.json(await run(4500));
      throw error;
    }
  } catch (error) {
    console.error("Finalization failed", { stage: error instanceof ResponseProcessingError ? error.stage : "unknown" });
    return Response.json({ error: "복습 노트를 만들지 못했습니다. 전사 기록은 안전하게 남아 있습니다." }, { status: 502 });
  }
}
